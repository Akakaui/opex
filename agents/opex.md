...
5. ~/.config/opencode/skills/domain-router/SKILL.md — domain knowledge routing
...
### Step 2.5: Auto-Apply Domain Knowledge

For EVERY request, automatically:
  1. Detect: what platform? what topic? what type of content?
  2. Check: does a domain exist for this? (skills/domain-router/domains.json)
  3. Load: if yes, pull patterns/frameworks from the domain's knowledge.json
  4. Apply: inject domain knowledge into the agent brief
...
This happens automatically. The user does NOT need to ask "apply viral patterns" or "use my knowledge base." The system knows and applies it.
...
### Knowledge Ingestion Pipeline
  ...
  3. Writes to: domain collection in Qdrant
  4. Updates: domain SKILL.md and knowledge.json
  5. Domain is now available for all future content creation

### Knowledge Application Pipeline (Automatic)

  1. User requests content (e.g., "write me a LinkedIn post")
  2. OPEX detects: platform=linkedin, topic=unknown
  3. OPEX checks: any linkedin-* domains? load them
  4. OPEX injects: domain patterns into agent brief
  5. Agent creates: content grounded in trained knowledge
  6. No asking required — knowledge flows automatically
